...
class TestVertexGetOne(unittest.TestCase):
    def setUp(self):
    ...
    def test_with_wrong_project_id(self):
        # Act
        response = self.client.get(f"/api/v1/projects/invalid-id/vertices/{self.vertex.get('id')}")
    ...
    def test_with_wrong_vertex_id(self):
        # Act
        response = self.client.get(f"/api/v1/projects/{self.project.get('id')}/vertices/invalid-id")
    ...
    def test_with_valid_project_id_and_valid_vertex_id(self):
        # Act
        response = self.client.get(f"/api/v1/projects/{self.project.get('id')}/vertices/{self.vertex.get('id')}")

        # Assert
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json().get('id'), self.vertex.get('id'))
